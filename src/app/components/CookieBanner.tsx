...
export default function CookieBanner({ onAccept, onReject }: CookieBannerProps) {
  ...
  return (
    <aside aria-label={t("common.cookies.title")} className="fixed bottom-0 left-0 right-0 z-[100] bg-primary text-primary-foreground p-4 md:p-5 shadow-2xl">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center gap-4 justify-between">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm leading-relaxed opacity-90">
            ...
            <Link to={path("/politica-de-cookies")} className="underline hover:text-accent transition-colors">
              {t("common.footer.links.cookies")}
            </Link>.
          </p>
        </div>
